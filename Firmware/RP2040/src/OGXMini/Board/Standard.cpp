 ...
 * repeating_timer (pio_usb_sof_timer_cb) is the one calling pio_usb_host_frame(), so other
 * call sites (HostManager::send_feedback, tuh_xinput wait_for_tx_complete) must not also call it.
 */
bool sof_timer_active() {
    return s_pio_usb_sof_hw_timer;
}
...
#endif // PI_PICO || RP2040_ZERO || ADAFRUIT_FEATHER || RP2350_USB_A || RP2350_ZERO || RP2040_XIAO    if (s_pio_usb_sof_hw_timer) {
        return;
    }
    static absolute_time_t next_pio_frame;
    static bool next_inited = false;
